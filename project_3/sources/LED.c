 ...
 *
 *  	Filename: LED.c
 *      Description:  This source file contains the functions for LED
 *
...
void LED_INTENSITY(uint8_t color, uint16_t intensity){
	if(color == OFF){
		...
		TPM_CnV_REG(TPM2_BASE_PTR,0) = intensity;
		TPM_CnV_REG(TPM2_BASE_PTR,1) = intensity;
		TPM_CnV_REG(TPM0_BASE_PTR,1) = 00;
	}
	else if(color == MAGENTA){
		TPM_CnV_REG(TPM2_BASE_PTR,0) = intensity;
		TPM_CnV_REG(TPM2_BASE_PTR,1) = 00;
		TPM_CnV_REG(TPM0_BASE_PTR,1) = intensity;
	}
	else if(color == GREEN){
		TPM_CnV_REG(TPM2_BASE_PTR,0) = 00;
		TPM_CnV_REG(TPM2_BASE_PTR,1) = intensity;
